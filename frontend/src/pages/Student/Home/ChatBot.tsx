...
const HelpDesk: FC<Props> = ({ delay }) => {
    ...
    const generateChatSubmit = async (content: string) => {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                newArr.pop();
                newArr.push({ query: content, output: "Name: Swati Khullar, Email: khullarswati@gmail.com, Experience: 2 years, Price: 5500" });
    ...
    const generateChatSubmit2 = async (content: string) => {
        setIsFetchingChat(true);
        try {
            const arr = generatedChat;
            arr.push({ query: content, output: "" });
            ...
            setTimeout(() => {
                const newArr = generatedChat;
                newArr.pop();
                newArr.push({ query: content, output: "5 years" });
                setGeneratedChat(newArr);
                setChat("");
                setIsFetchingChat(false);
            }, 3000)
            return;
            const response = await fetchTutorDetails(content);
            if (response.status === 200) {
                const newArr = generatedChat;
                newArr.pop();
                newArr.push({ query: content, output: response.data });
    ...
    const generateChatSubmit3 = async (content: string) => {
        setIsFetchingChat(true);
        try {
            const arr = generatedChat;
            arr.push({ query: content, output: "" });
            ...
            setTimeout(() => {
                const newArr = generatedChat;
                newArr.pop();
                newArr.push({ query: content, output: "मैं खाना खाऊंगा" });
                setGeneratedChat(newArr);
                setChat("");
                setIsFetchingChat(false);
            }, 3000)
            return;
            const response = await languageQuestionAnswer(content);
            if (response.status === 200) {
                const newArr = generatedChat;
                newArr.pop();
                newArr.push({ query: content, output: response.data });
                                ...
                                <div style={{ width: "75%" }}>
                                    <Flex className="flex-col">
                                        <SegmentedControl
                                            ...
                                            data={[
                                                { label: "Tutor recommendation", value: "1" },
                                                { label: "Tutor details", value: "2" },
                                                { label: "Language question answering", value: "3" },
                                            ]}
                                        />
                                    </Flex>
                                ...
                                >
                                    {
                                        (isFetchingChat ? (
                                            ...
                                            <Button
                                                w={"100%"}
                                                disabled={isFetchingChat}
                                                onClick={() => { if (value == "1") { generateChatSubmit(chat) } else if (value == "2") { generateChatSubmit2(chat) } else { generateChatSubmit3(chat) } }}
                                            >
                                                {" "}
                                                Generate
                                            </Button>
